...
fn plic_enable(source_id: usize, context_id: u64) {
    let source_chunk = source_id / 32;
    let source_bit = source_id % 32;
    let p = (PLIC_ENABLE_BASE + context_id as usize * 0x80 + source_chunk) as *mut u32;
    unsafe { p.write_volatile(p.read_volatile() | (1 << source_bit)) };
}
